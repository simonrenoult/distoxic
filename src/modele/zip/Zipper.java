...
public class Zipper
{
	...
	public boolean zipAction()
	{
		try
		{
			...
			// cr�ation d'un buffer d'ecriture
			BufferedOutputStream buff = new BufferedOutputStream(checksum);

			// cr�ation d'un flux d'ecriture Zip
			ZipOutputStream out = new ZipOutputStream(buff);

			// sp�cification de la methode de compression
			out.setMethod(ZipOutputStream.DEFLATED);

			// sp�cifier la qualite de la compression 0..9
			...
			for (int i = 0 ; i < files.length ; i++)
			{
				...
				// creation d'en entr�ee Zip pour ce fichier
